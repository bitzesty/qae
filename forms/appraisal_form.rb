class AppraisalForm
  ...
  RAG_ALLOWED_VALUES = %w[
    negative
    average
    positive
  ]

  NON_RAG_ALLOWED_VALUES = %w[
    negative
    average
    positive
  ]

  STRENGTHS_ALLOWED_VALUES = %w[
    neutral
    negative
    average
    positive
  ...
  def self.all
    ...
    AppraisalForm::SUPPORTED_YEARS.map do |year|
      const_get("ALL_FORMS_#{year}").each do |form|
        form.each do |k, obj|
          out << rate(k).to_sym if %i[strengths rag non_rag verdict].include?(obj[:type])
          # strengths doesn't have description
          out << desc(k).to_sym if %i[rag non_rag verdict].include?(obj[:type])
        end
      end
