...
describe "QaePdfForms::Awards2014::Development::Base" do
  let!(:user) do
  ...
  let(:development_award_form_answer) do
    FactoryGirl.create :form_answer, :submitted, :development, 
      user: user,
      document: step1_question_answers.merge(step2_question_answers)
  end

  let(:award_form) { development_award_form_answer.award_form }
  let(:steps) { award_form.decorate.steps }

  let(:pdf_generator) do 
    development_award_form_answer.decorate.pdf_generator
  end
  ...
  describe "PDF generation" do 
    ...
    let(:award_application_title) do
      development_award_form_answer.decorate.award_application_title
    end
    ...
    let(:form_urn) do
      development_award_form_answer.urn
    end
