...
describe Reports::Dashboard::ApplicationsReport do
  ...
  let!(:application_2) do
    create(:form_answer, :trade, :submitted, created_at: deadline - 2.months - 1.hour, submitted_at: deadline - 2.months - 1.hour)
  end
  let!(:application_3) do
    create(:form_answer, :trade, :submitted, created_at: deadline - 5.weeks - 1.hour, submitted_at: deadline - 4.weeks - 1.hour)
  end
  let!(:application_4) do
    create(:form_answer, :trade, :submitted, created_at: deadline - 4.weeks - 1.hour, submitted_at: deadline - 4.weeks - 1.hour)
  end
  let!(:application_5) do
    create(:form_answer, :trade, :submitted, created_at: deadline - 2.days - 1.hour, submitted_at: deadline - 1.day - 1.hour)
  end
