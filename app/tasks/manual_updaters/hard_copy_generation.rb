    ...
    class << self
      def case_summary_individual
        ...
        year.hard_copy_case_summary_scope.find_each do |form_answer|
          ...
          logy "[#{ENV.fetch("MAILER_HOST", nil)} | CS IND | #{form_answer.id}] -------------------------------- updated"
        rescue StandardError => e
          not_updated_entries_cs_individual << form_answer.id

          logy "[#{ENV.fetch("MAILER_HOST", nil)} | CS IND | #{form_answer.id} | ERROR] --------------------------------- #{e.message}"
        end

        if not_updated_entries_cs_individual.present?
          logy "[#{ENV.fetch("MAILER_HOST",
                             nil)} | CS IND | errored #{not_updated_entries_cs_individual.count}] ------------ #{not_updated_entries_cs_individual.inspect}"
        end

        if year.check_hard_copy_pdf_generation_status!("case_summary")
          logy "[#{ENV.fetch("MAILER_HOST", nil)} | CS IND] -------------- DONE SUCCESSFULY!"
        else
          ...
          logy "[#{ENV.fetch("MAILER_HOST", nil)} | CS IND | ERRORS] -------------- STATS DIDN'T MATCH!"
          logy "[#{ENV.fetch("MAILER_HOST", nil)} | CS IND | ERRORS] -------------- scope: #{stats} | updated_scope: #{updated_stats}"
        end
      end
      ...
      def feedback_individual
        ...
        year.hard_copy_feedback_scope.find_each do |form_answer|
          ...
          logy "[#{ENV.fetch("MAILER_HOST", nil)} | FEED IND | #{form_answer.id}] -------------------------------- updated"
        rescue StandardError => e
          not_updated_entries_feed_individual << form_answer.id

          logy "[#{ENV.fetch("MAILER_HOST", nil)} | FEED IND | #{form_answer.id} | ERROR] --------------------------------- #{e.message}"
        end

        if not_updated_entries_feed_individual.present?
          logy "[#{ENV.fetch("MAILER_HOST",
                             nil)} | FEED IND | errored #{not_updated_entries_feed_individual.count}] ------------ #{not_updated_entries_feed_individual.inspect}"
        end

        if year.check_hard_copy_pdf_generation_status!("feedback")
          logy "[#{ENV.fetch("MAILER_HOST", nil)} | FEED IND] -------------- DONE SUCCESSFULY! STATS MATCHES!"
        else
          ...
          logy "[#{ENV.fetch("MAILER_HOST", nil)} | FEED IND | ERRORS] -------------- STATS DIDN'T MATCH!"
          logy "[#{ENV.fetch("MAILER_HOST", nil)} | FEED IND | ERRORS] -------------- scope: #{stats} | updated_scope: #{updated_stats}"
        end
      end
