module AppraisalFormHelper
  ...
  ENTRY_RELATES_TO_TRADE_OPS = %w[products services]
  ENTRY_RELATES_TO_INNOVATION_OPS = %w[product service business_model]
  ENTRY_RELATES_TO_DEVELOPMENT_OPS = %w[product service management_approach]
  ENTRY_RELATES_TO_MOBILITY_OPS = %w[mentoring career_opportunities_accessibility workplace_fostering]
  ...
  def render_section(form_answer, f)
    ...
    AppraisalForm.struct(form_answer, f).map do |k, section|
      ...
      render partial:, locals: { section: section_obj, f:, section_odd_even: }
    end.join.html_safe
  end
end
