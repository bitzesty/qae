  ...
  if ($ '.eligibility_current_holder_of_qae_for_trade').length
    toggleOther = () ->
      if ($ '#eligibility_current_holder_of_qae_for_trade_true').is(':checked')
        ($ '.eligibility_qae_for_trade_expiery_date').closest(".question").removeClass('visuallyhidden')
      else
        ($ '.eligibility_qae_for_trade_expiery_date').closest(".question").addClass('visuallyhidden')
