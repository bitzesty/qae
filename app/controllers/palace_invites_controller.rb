class PalaceInvitesController < ApplicationController
  ...
  def palace_invite_attributes
    return if params[:palace_invite].blank?

    params.require(:palace_invite).permit(
      palace_attendees_attributes: %i[
        title
        first_name
        last_name
        job_name
  ...
  def require_palace_invite_to_be_not_submitted_and_proper_stage!
    return if Settings.buckingham_palace_invites_stage?(@invite.form_answer.award_year.settings)
  ...
  def check_reception_attendee_information_deadline!
    ...
    unless @invite.form_answer.award_year.fetch_deadline("buckingham_palace_reception_attendee_information_due_by")&.trigger_at&.past?
      return
    end
