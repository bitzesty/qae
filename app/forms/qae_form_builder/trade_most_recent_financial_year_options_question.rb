    ...
    def errors
      ...
      if [from_ts, to_ts].all?(&:present?)
        ...
        if date.present? && !date.in?(range) && question.year_has_changed?
          result[question.key] = "You can only the year if your dates in question D2 range between #{from.decorate.formatted_trigger_date} to #{to.decorate.formatted_trigger_date}."
        end
      end
