class AssessorAssignment < ApplicationRecord
  has_paper_trail unless: proc { |_t| Rails.env.test? }
  ...
  def visible_for?(subject)
    ...
    return true if subject.is_a?(Assessor) && form_answer.from_previous_years?
    ...
    return assessments.any? { |a| a.assessor_id == subject.id } if assessments.all?(&:submitted?)
  ...
  def mandatory_fields_for_submitted
    ...
    struct.meths_for_award_type(form_answer, moderated?).each do |meth|
      next if public_send(meth).present?
  ...
  def section_name(key)
    ...
    return unless section = @_sections.dig(section_key.to_sym)
  ...
  def assessor_existence
    return unless (moderated? || case_summary?) && assessor_id.present?
  ...
  def assessor_assignment_to_category
    ...
    return unless assessor.present? && !assessor.assignable?(form_answer)
  ...
  def notify_form_answer
    ...
    assessor_changed = assessor_id_changed? if primary? || secondary?

    yield

    return unless assessor_changed
