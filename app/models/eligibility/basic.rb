class Eligibility::Basic < Eligibility
  ...
  property :industry, values: %w[product_business service_business either_business], label: "Is your business mainly a:", accept: :not_nil_or_charity, if: proc { (kind_value.nil? || kind.application?) && (!organization_kind_value || !organization_kind.charity?) }
  property :registered, boolean: true, label: "Is it registered with the UK Government?", accept: :not_nil, if: proc { kind_value.nil? || kind.application? }
  property :self_contained_enterprise, boolean: true, label: "Does it act as a self-contained operational unit?", accept: :true, if: proc { (kind_value.nil? || kind.application?) && !registered? }
  property :current_holder, boolean: true, label: "Are you a current Queen's Award holder in any category?", accept: :not_nil, if: proc { kind_value.nil? || kind.application? }
