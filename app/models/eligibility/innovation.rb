class Eligibility::Innovation < Eligibility
  ...
  property :innovative_product,
            ...
            label: "Do you have an innovative product/service/initiative?",
            accept: :true,
            hint: 'A product/service/initiative is innovative if it has not been done before, and provides benefit/ solves prior problems for its users. Fields include invention, design and production (for goods), performance (for services), marketing, distribution, after-sale support.'

  property :number_of_innovative_products,
            positive_integer: true,
            label: "How many innovative products/services/initiatives do you have?",
            accept: :not_nil,
            if: proc { !skipped? && (innovative_product.nil? || innovative_product?) }

  property :was_on_market_for_two_years,
            boolean: true,
            label: "Has it been on the market for at least two years?",
            accept: :true,
            if: proc { !skipped? }
